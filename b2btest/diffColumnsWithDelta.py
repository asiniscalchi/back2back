...
class DiffColumnsWithDelta(DiffBase) : 
	def __init__(self) : 
		DiffBase.__init__(self)
		self._extraArgsDefaultsDict.update({ "metadata_threshold" : 1E-5 })
		self._validExtensions = [ 'metadata', 'txt' ]
	...
	def differences(self, expected, result, diffbase, input_extra_args) :
		...
		delta = float(extra_args["metadata_threshold"])
